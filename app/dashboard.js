...
import { ArrowLeft, MousePointer2, RefreshCw, Calendar } from "lucide-react";
...
// --- CUSTOM TOOLTIP (Fixed to not block clicks) ---
    ...
    return (
      <div
        style={{
          ...
          // CRITICAL FIX: Allows clicks to pass through the tooltip to the bar below
          pointerEvents: "none",
        }}
      >
        <p
...
export default function Dashboard() {
  ...
  const [selectedDay, setSelectedDay] = useState(null); // Holds the data object of the clicked bar

  // Derived Metrics (calculated based on selection)
  ...
  useEffect(() => {
    let logs = [];
    try {
      logs = JSON.parse(localStorage.getItem("all_logs") || "[]");
    } catch {}
    setAllLogs(logs);
    processData(logs, timeRange);
  }, [timeRange]);
  ...
  const processData = (logs, range) => {
    ...
    setSelectedDay(null); // Reset selection when changing range
  };

  // 3. Filter logs for the list view
        ...
        <div style={{ width: "100%", height: 260 }}>
          <ResponsiveContainer>
            ...
            >
              ...
              {/* FIXED INTERACTION: onClick is now on the Bar itself */}
              <Bar
                dataKey="Calories"
                radius={[4, 4, 4, 4]}
                cursor="pointer"
                onClick={(data) => {
                  // data contains the payload for the clicked bar
                  setSelectedDay(data);
                }}
              >
