...
export default function UserDashboard() {
  ...
  const generateInsights = (eaten, targets, macros, dailyLogs, historyLogs) => {
    ...
    if (fatDiff > 5) { 
      const culprit = dailyLogs.reduce((prev, current) => 
        ((prev.fats || 0) > (current.fats || 0)) ? prev : current, { fats: 0, name: 'Unknown' }
      );
      if (culprit.id) newCulprits.add(culprit.id);
      suggestions.push({
        type: "danger",
        title: "High Fat Alert",
        msg: `You exceeded fats by ${Math.round(fatDiff)}g. Main cause: ${culprit.name} (${culprit.fats}g).`,
    ...
    if (carbDiff > 20) {
       const culprit = dailyLogs.reduce((prev, current) => 
        ((prev.carbs || 0) > (current.carbs || 0)) ? prev : current, { carbs: 0, name: 'Unknown' }
      );
      if (culprit.id) newCulprits.add(culprit.id);
      suggestions.push({
        type: "warn",
        title: "Carb Limit Exceeded",
        msg: `Carbs are high (+${Math.round(carbDiff)}g). The ${culprit.name} contributed ${culprit.carbs}g.`,
      });
    }
    ...
    if (proteinMissed > 15) {
      ...
      suggestions.push({
        ...
        fix: smartFix || "Try: 1 Scoop Whey (25g), 150g Chicken (30g), or Greek Yogurt.",
      });
    }
    ...
    if (calDiff > 500 && suggestions.length === 0) {
       const culprit = dailyLogs.reduce((prev, current) => 
        ((prev.calories || 0) > (current.calories || 0)) ? prev : current, { calories: 0, name: 'Unknown' }
      );
      if (culprit.id) newCulprits.add(culprit.id);
      ...
      suggestions.push({
        ...
        msg: `+${Math.round(calDiff)} kcal over limit. Heaviest item: ${culprit.name} (${culprit.calories} kcal).`,
      });
    } else if (calDiff < -500) {
       suggestions.push({
        type: "warn",
  ...
  const handleDateSelect = (dateStr, logsSource, preCalcTargets) => {
    ...
    const analysis = generateInsights(eatenCals, targets, macrosEaten, dailyLogs, logsToFilter);
  ...
  return (
    ...
    >
      ...
      {!profile ? (
        <div style={{ background: "#27272a", padding: 30, borderRadius: 12, textAlign: "center", color: "#aaa" }}>
        ...
        <>
          ...
          >
            ...
            ></div>
            <div style={{ fontSize: "0.85rem", color: "#888", textTransform: "uppercase", letterSpacing: 2, fontWeight: 600 }}>
              Energy Balance ({selectedDate === new Date().toISOString().slice(0, 10) ? "Today" : selectedDate})
            </div>
            <div style={{ fontSize: "3rem", fontWeight: 800, color: metrics.statusColor, margin: "10px 0", lineHeight: 1 }}>
              {metrics.status}
            </div>
            <div style={{ color: "#ccc", fontSize: "1.1rem" }}>
              {metrics.eaten} <span style={{ color: "#666" }}>/ {metrics.target} kcal</span>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 20, marginBottom: 20 }}>
             {/* 3. CONSISTENCY */}
            <div className="chart-card" style={{ padding: 24, display: "flex", flexDirection: "column" }}>
               {/* ... (Existing Consistency Code) ... */}
              <h3 style={{ fontSize: "1.1rem", fontWeight: 700, marginBottom: 15, display: "flex", gap: 8 }}>
                ...
                {calendarData.map((day, i) => (
                  <div
                    ...
                    style={{
                      ...
                      border: day.date === selectedDate ? "2px solid white" : "1px solid transparent",
                      transition: "all 0.2s",
                    }}
            ...
            <div className="chart-card" style={{ padding: 24, display: "flex", flexDirection: "column", gridColumn: "span 2" }}>
              <h3 style={{ fontSize: "1.1rem", fontWeight: 700, marginBottom: 15, display: "flex", gap: 8 }}>
                <Lightbulb size={18} color="#FFC107" /> Smart Analysis & Remedies
              </h3>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 15 }}>
                {insights.map((insight, i) => (
                  <div
                    key={i}
                    style={{
                      background: insight.type === "success" ? "rgba(34, 197, 94, 0.1)" : insight.type === "danger" ? "rgba(239, 68, 68, 0.1)" : "rgba(245, 158, 11, 0.1)",
                      border: `1px solid ${insight.type === "success" ? "#22c55e" : insight.type === "danger" ? "#ef4444" : "#f59e0b"}`,
                      padding: 15,
                      borderRadius: 12,
                    }}
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      {insight.type === "success" && <CheckCircle size={20} color="#22c55e" />}
                      {insight.type === "warn" && <AlertTriangle size={20} color="#f59e0b" />}
                      {insight.type === "danger" && <AlertCircle size={20} color="#ef4444" />}
                      {insight.type === "tip" && <TrendingUp size={20} color="#3b82f6" />}
                      <span style={{ fontWeight: 700, color: "#fff" }}>{insight.title}</span>
                    </div>
                    <p style={{ fontSize: "0.9rem", color: "#ccc", margin: "0 0 8px 0", lineHeight: 1.4 }}>{insight.msg}</p>
                    {insight.fix && (
                      <div style={{ fontSize: "0.85rem", color: "#fff", background: "rgba(255,255,255,0.05)", padding: "6px 10px", borderRadius: 6, display: "inline-block" }}>
            ...
            {/* 2. HYDRATION */}
            <div className="chart-card" style={{ padding: 24, display: "flex", flexDirection: "column", justifyContent: "center" }}>
              {/* ... (Existing Hydration Code) ... */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <h3 style={{ fontSize: "1.1rem", display: "flex", alignItems: "center", gap: 10, fontWeight: 700 }}>
                  <Droplets size={20} color="#3b82f6" /> Hydration
                </h3>
                <span style={{ fontSize: "0.9rem", color: "#3b82f6", background: "rgba(59, 130, 246, 0.1)", padding: "4px 8px", borderRadius: 6, fontWeight: 600 }}>
                  {Math.round((metrics.water.current / metrics.water.target) * 100) || 0}%
                </span>
              </div>
              <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 16 }}>
                <span style={{ fontSize: "2.5rem", fontWeight: 800, color: "#fff" }}>{metrics.water.current}L</span>
                <span style={{ fontSize: "1.2rem", color: "#666" }}>/ {metrics.water.target}L</span>
              </div>
              <div style={{ background: "#27272a", height: 16, borderRadius: 8, overflow: "hidden", position: "relative" }}>
                <div style={{ width: `${Math.min(100, (metrics.water.current / metrics.water.target) * 100)}%`, height: "100%", background: "#3b82f6", transition: "width 1s ease" }}></div>
              </div>
            </div>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 20, marginBottom: 20 }}>
            ...
            {/* MACRO CHART */}
            <div className="chart-card" style={{ padding: 24, flex: "1 1 300px", height: 500, display: "flex", flexDirection: "column" }}>
              {/* ... (Existing Macro Chart Code) ... */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
                <h3 style={{ fontSize: "1.1rem", fontWeight: 700 }}>Macro Targets</h3>
                <span style={{ fontSize: "0.7rem", color: "#666" }}>For {selectedDate}</span>
              </div>
              <div style={{ flex: 1, width: "100%", minHeight: 0 }}>
                <ResponsiveContainer>
                  <PieChart key={JSON.stringify(macroData)}>
                    ...
                    <Tooltip contentStyle={{ background: "#000", border: "1px solid #333", borderRadius: 8 }} itemStyle={{ color: "#fff" }} />
                  </PieChart>
                </ResponsiveContainer>
              </div>
              {/* Macro stats list */}
              <div style={{ display: "grid", gap: 12, marginTop: 20 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #222" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.9rem" }}> <div style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS.pro }}></div> Protein </span>
                  <span style={{ fontSize: "0.9rem", fontWeight: 600 }}> {Math.round(metrics.macros.p)} <span style={{ color: "#666", fontWeight: 400 }}> / {metrics.targets.p}g </span> </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderTop: "1px solid #222" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.9rem" }}> <div style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS.fib }}></div> Fiber </span>
                  <span style={{ fontSize: "0.9rem", fontWeight: 600 }}> {Math.round(metrics.macros.fib)} <span style={{ color: "#666", fontWeight: 400 }}> / {metrics.targets.fib}g </span> </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid #222" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.9rem" }}> <div style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS.carb }}></div> Carbs </span>
                  <span style={{ fontSize: "0.9rem", fontWeight: 600 }}> {Math.round(metrics.macros.c)} <span style={{ color: "#666", fontWeight: 400 }}> / {metrics.targets.c}g </span> </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0" }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: "0.9rem" }}> <div style={{ width: 8, height: 8, borderRadius: "50%", background: COLORS.fat }}></div> Fats </span>
                  <span style={{ fontSize: "0.9rem", fontWeight: 600 }}> {Math.round(metrics.macros.f)} <span style={{ color: "#666", fontWeight: 400 }}> / {metrics.targets.f}g </span> </span>
                </div>
              </div>
            </div>

            {/* INTAKE LIST (WITH CULPRIT FINDER & SCROLLBAR) */}
            <div className="chart-card" style={{ padding: 24, flex: "2 1 400px", height: 500, display: "flex", flexDirection: "column" }}>
              ...
              {!selectedDate || selectedLogs.length === 0 ? (
                ...
                <>
                  ...
                  <div className="custom-scrollbar" style={{ flex: 1, overflowY: "auto", paddingRight: 6 }}>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            background: isCulprit ? "rgba(239, 68, 68, 0.1)" : "transparent",
                            borderLeft: isCulprit ? "3px solid #ef4444" : "3px solid transparent",
                            borderRadius: 4,
                            marginBottom: 4,
                          }}
                        >
                          <div>
                            <div style={{ fontWeight: 600, color: isCulprit ? "#ef4444" : "#fff", textTransform: "capitalize", fontSize: "1rem" }}>
                              {log.qty}x {log.name} {log.name === "Water" && <span style={{ fontSize: "0.8rem", color: "#3b82f6" }}> ({log.qty * 0.25}L)</span>}
                            </div>
                            {log.name !== "Water" && (
                              <div style={{ fontSize: "0.8rem", color: "#888", marginTop: 4 }}>
                                <span style={{ color: COLORS.pro }}>P: {log.protein}</span> •{" "}
                                <span style={{ color: COLORS.carb }}>C: {log.carbs}</span> •{" "}
                                <span style={{ color: COLORS.fat, fontWeight: isCulprit && log.fats > 10 ? '800' : '400' }}>F: {log.fats}</span> •{" "}
                                <span style={{ color: COLORS.fib }}>
                                  Fib: {log.fiber || (FLATTENED_DB[log.name.toLowerCase()]?.fiber ? Math.round(FLATTENED_DB[log.name.toLowerCase()].fiber * log.qty) : 0)}
                                </span>
                              </div>
                            )}
                          </div>
                          <div style={{ fontWeight: 700, color: "#ddd", fontSize: "1rem" }}>
                            {log.calories} kcal
                          </div>
                        </div>
                      );
                  ...
                  {/* FIXED FOOTER */}
                  <div style={{ marginTop: 15, paddingTop: 15, borderTop: "2px solid #333", display: "flex", justifyContent: "space-between", fontWeight: 800, color: "#fff", fontSize: "1.1rem", flexShrink: 0 }}>
                    <span>Daily Total</span>
                    <span>{selectedLogs.reduce((sum, item) => sum + (item.calories || 0), 0)} kcal</span>
                  </div>
                </>
              )}
            </div>
